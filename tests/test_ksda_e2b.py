...
def test_e2b_parser_accepts_all_roots():
    from experiments import ksda_exp_e2b_soft_rbid

    parser = ksda_exp_e2b_soft_rbid.build_argparser()
    ...
    assert args.e2a_root == "results/e2a/demo"
    assert args.refresh_root == "results/e2_diag/demo"


def test_e2b_single_fold_smoke_and_outputs(tmp_path: Path):
    from experiments.ksda_exp_e2b_soft_rbid import (
        DEFAULT_ALIGNER_CFG,
        DEFAULT_REP_CFG,
        build_summary,
        evaluate_e2b_loso,
    ...
    loso_df, pairwise_df, method_row = evaluate_e2b_loso(
        folds=[fold],
    ...
    assert method_row["method"] == "RBID-aware Conservative Koopman aligner-r48 (Soft-RBID)"
    ...
    e2a_summary = {
        "e2a_proxy_replacement": {
            "method": "RBID-aware Conservative Koopman aligner-r48 (mean+dyn proxy)",
            "rbid": 0.3095,
            "tail_rbid": 0.6327,
            "pearson_r": 0.3696,
            "pairwise_accuracy_mean": 0.3146,
        }
    }
    refresh_summary = {
    ...
    assert "secondary_control_e2a" in saved
    assert "delta_vs_e2a_pairwise" in saved
    assert saved["rank_loss_mode"] == "soft_rbid_huber"
