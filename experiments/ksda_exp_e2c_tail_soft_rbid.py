...
logger = get_logger("ksda_exp_e2c_tail_soft_rbid")
...
METHOD_NAME = "RBID-aware Conservative Koopman aligner-r48 (Tail-Soft-RBID)"
METHOD_KEY = "rbid-aware-conservative-koopman-aligner-r48-tail-soft-rbid"
E1_CONTROL_METHOD = "Conservative Koopman aligner-r48"
E2_CONTROL_METHOD = "RBID-aware Conservative Koopman aligner-r48"
E2A_CONTROL_METHOD = "RBID-aware Conservative Koopman aligner-r48 (mean+dyn proxy)"
E2B_CONTROL_METHOD = "RBID-aware Conservative Koopman aligner-r48 (Soft-RBID)"
REFRESH_CONTROL_METHOD = "Conservative Koopman aligner-r48 (target-global refresh)"


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser()
    parser.add_argument("--run-name", default="2026-03-09-e2c-r1")
    ...
    parser.add_argument("--e2a-root", default="results/e2a/2026-03-09-e2a-r1")
    parser.add_argument("--e2b-root", default="results/e2b/2026-03-09-e2b-r1")
...
def _make_aligner(aligner_cfg: Dict[str, object]) -> KoopmanConservativeResidualAligner:
    return KoopmanConservativeResidualAligner(
        ...
        rank_loss_mode=str(aligner_cfg["rank_loss_mode"]),
        rank_mean_weight=float(aligner_cfg["rank_mean_weight"]),
        rank_dyn_weight=float(aligner_cfg["rank_dyn_weight"]),
        rank_tau=float(aligner_cfg["rank_tau"]),
        rank_huber_delta=float(aligner_cfg["rank_huber_delta"]),
        rank_tail_weight=float(aligner_cfg["rank_tail_weight"]),
        rank_tail_quantile=float(aligner_cfg["rank_tail_quantile"]),
...
def build_summary(
    *,
    ...
    e2a_summary: Dict[str, object],
    e2b_summary: Dict[str, object],
    refresh_summary: Dict[str, object],
    run_dir: Path,
) -> Dict[str, object]:
    e1_control = dict(e1_summary["conservative"])
    e2_control = dict(e2_summary["rbid_aware"])
    e2a_control = dict(e2a_summary["e2a_proxy_replacement"])
    e2b_control = dict(e2b_summary["e2b_soft_rbid"])
    ...
    e2a_pairwise_accuracy_mean = float(e2a_control.get("pairwise_accuracy_mean", e2a_control.get("accuracy_mean", 0.0)))
    e2b_pairwise_accuracy_mean = float(e2b_control.get("pairwise_accuracy_mean", e2b_control.get("accuracy_mean", 0.0)))
    ...
    return {
        ...
        "rank_loss_mode": str(DEFAULT_ALIGNER_CFG["rank_loss_mode"]),
        "rank_mean_weight": float(DEFAULT_ALIGNER_CFG["rank_mean_weight"]),
        "rank_dyn_weight": float(DEFAULT_ALIGNER_CFG["rank_dyn_weight"]),
        "rank_tau": float(DEFAULT_ALIGNER_CFG["rank_tau"]),
        "rank_huber_delta": float(DEFAULT_ALIGNER_CFG["rank_huber_delta"]),
        "rank_tail_weight": float(DEFAULT_ALIGNER_CFG["rank_tail_weight"]),
        "rank_tail_quantile": float(DEFAULT_ALIGNER_CFG["rank_tail_quantile"]),
        ...
        "secondary_control_e2a": {**e2a_control, "pairwise_accuracy_mean": e2a_pairwise_accuracy_mean},
        "secondary_control_e2b": {**e2b_control, "pairwise_accuracy_mean": e2b_pairwise_accuracy_mean},
        "e2c_tail_soft_rbid": {
            "method": METHOD_NAME,
        ...
        "delta_vs_e2a_pairwise": {
            "rbid": rbid - float(e2a_control["rbid"]),
            "tail_rbid": tail_rbid - float(e2a_control["tail_rbid"]),
            "pearson_r": float(method_row["pearson_r"]) - float(e2a_control["pearson_r"]),
            "pairwise_accuracy_mean": pairwise_accuracy_mean - e2a_pairwise_accuracy_mean,
        },
        "delta_vs_e2b_pairwise": {
            "rbid": rbid - float(e2b_control["rbid"]),
            "tail_rbid": tail_rbid - float(e2b_control["tail_rbid"]),
            "pearson_r": float(method_row["pearson_r"]) - float(e2b_control["pearson_r"]),
            "pairwise_accuracy_mean": pairwise_accuracy_mean - e2b_pairwise_accuracy_mean,
        },
        "gate": {
...
def write_outputs(
    *,
    ...
    rank_components_df: pd.DataFrame | None = None,
    ...
    if rank_components_df is not None:
...
def _build_pairwise_panel(
    ...
    e2a_pairwise = pd.read_csv(e2a_root / "pairwise_scores.csv")
    e2a_pairwise = e2a_pairwise.loc[e2a_pairwise["method"] == E2A_CONTROL_METHOD].copy()
    e2b_pairwise = pd.read_csv(e2b_root / "pairwise_scores.csv")
    e2b_pairwise = e2b_pairwise.loc[e2b_pairwise["method"] == E2B_CONTROL_METHOD].copy()

    panel = pd.concat([e0_keep, refresh_pairwise, e2_pairwise, e2a_pairwise, e2b_pairwise, e2c_pairwise], ignore_index=True)
...
def _build_rbid_panel(
    ...
    e2a_rbid = pd.read_csv(e2a_root / "rbid_method_comparison.csv")
    e2a_rbid = e2a_rbid.loc[e2a_rbid["method"] == E2A_CONTROL_METHOD].copy()
    e2a_pairwise = pd.read_csv(e2a_root / "pairwise_scores.csv")
    e2a_pairwise = e2a_pairwise.loc[e2a_pairwise["method"] == E2A_CONTROL_METHOD].copy()
    if not e2a_rbid.empty and not e2a_pairwise.empty:
        e2a_rbid["pairwise_accuracy_mean"] = float(e2a_pairwise["accuracy"].mean())
    e2b_rbid = pd.read_csv(e2b_root / "rbid_method_comparison.csv")
    e2b_rbid = e2b_rbid.loc[e2b_rbid["method"] == E2B_CONTROL_METHOD].copy()
    e2b_pairwise = pd.read_csv(e2b_root / "pairwise_scores.csv")
    e2b_pairwise = e2b_pairwise.loc[e2b_pairwise["method"] == E2B_CONTROL_METHOD].copy()
    if not e2b_rbid.empty and not e2b_pairwise.empty:
        e2b_rbid["pairwise_accuracy_mean"] = float(e2b_pairwise["accuracy"].mean())
    e2c_rbid = pd.DataFrame([method_row])

    panel = pd.concat([e0_keep, refresh_rbid, e2_rbid, e2a_rbid, e2b_rbid, e2c_rbid], ignore_index=True)
...
def _render_memo(summary: Dict[str, object], loso_df: pd.DataFrame, rbid_df: pd.DataFrame) -> str:
    return (
        ...
        f"**Rank loss mode**: `{summary['rank_loss_mode']}`\n\n"
        ...
        + f"- RBID vs E2b: `{summary['delta_vs_e2b_pairwise']['rbid']:.4f}`\n"
...
def main(argv: list[str] | None = None) -> None:
    ...
    e2c_pairwise, e2c_method_row, rank_components = evaluate_e2c_pairwise(
        loader=loader,
    ...
    e2a_summary = json.loads((e2a_root / "summary.json").read_text(encoding="utf-8"))
    e2b_summary = json.loads((e2b_root / "summary.json").read_text(encoding="utf-8"))
    refresh_summary = json.loads((refresh_root / "summary.json").read_text(encoding="utf-8"))
    pairwise_panel = _build_pairwise_panel(e0_root, e2_root, e2a_root, e2b_root, refresh_root, e2c_pairwise)
    rbid_panel = _build_rbid_panel(e0_root, e2_root, e2a_root, e2b_root, refresh_root, e2c_method_row)
    summary = build_summary(
        loso_df=loso_df,
        method_row=e2c_method_row,
    ...
    logger.info("Wrote E2c tail-soft-rbid outputs to %s", run_dir)
