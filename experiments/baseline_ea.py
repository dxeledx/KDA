...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", default="results/baselines/ea")
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Running LOSO evaluation (EA)...")
    loso_df = evaluate_loso(
        loader, subjects, pre, csp_kwargs, lda_kwargs, method="ea", cov_eps=cov_eps
    )
    loso_df.to_csv(out_dir / "loso.csv", index=False)
    _save_summary(loso_df, out_dir / "summary.json")

    logger.info("Running pairwise transfer matrix (EA)...")
    ...
    plot_transfer_matrix(
        matrices["accuracy"],
        fig_dir / "transfer_matrix_ea.pdf",
        title="Cross-Subject Transfer Performance (EA)",
    )
