...
logger = get_logger("ksda_exp_e2b_soft_rbid")
...
DEFAULT_ALIGNER_CFG = {
    ...
    "rank_loss_mode": "soft_rbid_huber",
...
METHOD_NAME = "RBID-aware Conservative Koopman aligner-r48 (Soft-RBID)"
METHOD_KEY = "rbid-aware-conservative-koopman-aligner-r48-soft-rbid"
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--run-name", default="2026-03-09-e2b-r1")
...
def _build_pairwise_panel(
    ...
    panel = pd.concat([e0_keep, refresh_pairwise, e2_pairwise, e2a_pairwise, e2b_pairwise], ignore_index=True)
...
def _build_rbid_panel(
    ...
    e2b_rbid = pd.DataFrame([method_row])

    panel = pd.concat([e0_keep, refresh_rbid, e2_rbid, e2a_rbid, e2b_rbid], ignore_index=True)
...
def _render_memo(summary: Dict[str, object], loso_df: pd.DataFrame, rbid_df: pd.DataFrame) -> str:
    return (
        ...
        + f"- RBID vs E2a: `{summary['delta_vs_e2a_pairwise']['rbid']:.4f}`\n"
        + f"- Tail-RBID vs refresh: `{summary['delta_vs_refresh_pairwise']['tail_rbid']:.4f}`\n"
        + f"- Ready for E2c: `{summary['gate']['ready_for_e2c']}`\n"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    run_dir = ensure_dir(Path("results/e2b") / args.run_name)

    start = time.perf_counter()
    folds = _load_fold_cache(loader, all_subjects, target_subjects, pre, cov_eps)
    loso_df, _, _ = evaluate_e2b_loso(
        folds=folds,
    ...
    e2b_pairwise, e2b_method_row, rank_components = evaluate_e2b_pairwise(
        loader=loader,
    ...
    pairwise_panel = _build_pairwise_panel(e0_root, e2_root, e2a_root, refresh_root, e2b_pairwise)
    rbid_panel = _build_rbid_panel(e0_root, e2_root, e2a_root, refresh_root, e2b_method_row)
    summary = build_summary(
        loso_df=loso_df,
        method_row=e2b_method_row,
    ...
    logger.info("Wrote E2b soft-rbid outputs to %s", run_dir)
